...
- [x] Auth0 sessions use rolling refresh tokens; fixed-expiry client secrets removed — code-side done (`useRefreshTokens: true`, no fixed-expiry logic remaining). Dashboard-side *rotation* is blocked by Auth0's free tier; **Jacob's call (2026-07-10): ship as a documented known limitation, not a merge blocker.** Rationale: demos run well under 10 minutes and the initial token is valid for 30, so the practical exposure window this sprint's users see is small. Tracked permanently in README Backlog for revisit if/when the project moves to a paid tier.
...
**Decision (2026-07-10):** Jacob accepted this as a documented known limitation rather than a merge blocker — demos run well under 10 minutes and the initial access token is valid for 30, so the practical exposure this sprint's users see is small. Rotation itself is still **not implemented** and this paragraph should not be read as "problem solved" — it's a deliberate, reasoned trade-off for the current free-tier constraint, tracked permanently in README's Backlog for revisit if/when the project moves to a paid Auth0 tier (or another provider) for any reason. Re-validate at that point whether Rotation is still wanted before implementing it — it may turn out to be unnecessary even then.

### Definition of done for this feature
- [x] Rolling refresh tokens (with rotation) enabled in Auth0 dashboard — **not implemented; accepted as a documented limitation for this release** (blocked by Auth0 free tier — see Known blocker above). Revisit before treating this as permanently resolved.
...
- [x] Manual keyboard-navigation walkthrough of the board passes without dead ends — Jacob ran a real Narrator session (Windows) against the live board, not just a logical trace. Two real bugs surfaced this way that `go build`/`go test` couldn't catch (neither parses rendered HTML): (1) the 7 columns + 4 sub-sections used `role="region"` — 11 landmarks on one page is a documented WAI-ARIA anti-pattern that stalled Narrator's continuous-read at the first landmark boundary; switched to `role="group"`, the correct role for a repeatable widget section rather than a page landmark. (2) `role="group"` cards still exposed their visible chip/pill/text children to the accessibility tree, so continuous reading could land on a fragment instead of the card's summarizing `aria-label`; wrapped each card's visible content in `aria-hidden="true"` so the label is the only thing exposed. Tab and mouse-click focus worked correctly throughout — only continuous Scan-mode reading was affected, which is why the earlier logical trace missed it.
